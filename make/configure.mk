...
HOST_BUILDDIR              := $(BASEDIR)/.build/host
...
define srcconfig =
  ...
  $(if $(HOST_$(name)_INC),,     $(eval HOST_$(name)_INC     := $($(name)_PATH)/include))
  $(if $(HOST_$(name)_SRC),,     $(eval HOST_$(name)_SRC     := $($(name)_PATH)/src))
  $(if $(HOST_$(name)_TEST),,    $(eval HOST_$(name)_TEST    := $($(name)_PATH)/test))
  $(if $(HOST_$(name)_TESTING),, $(eval HOST_$(name)_TESTING := 0))
  $(if $(HOST_$(name)_BIN),,     $(eval HOST_$(name)_BIN     := $(HOST_BUILDDIR)/$($(name)_NAME)))
  $(if $(HOST_$(name)_CFLAGS),,  $(eval HOST_$(name)_CFLAGS  := "-I\"$($(name)_INC)\"" $(if $(builtin),"-D$(name)_BUILTIN")))
  $(if $(HOST_$(name)_LDLAGS),,  $(eval HOST_$(name)_LDFLAGS :=))
  $(if $(HOST_$(name)_OBJ),,     $(eval HOST_$(name)_OBJ     := "$($(name)_BIN)/$($(name)_NAME).o"))
  $(if $(HOST_$(name)_OBJ_TEST),,$(eval HOST_$(name)_OBJ_TEST:= "$($(name)_BIN)/$($(name)_NAME)-test.o"))
